...
public class MyFolderResource extends BaseResource implements FolderResource {
    ...
    public Resource createNew(String name, InputStream inputStream, Long totalLength, String mediaType) throws IOException, ConflictException, NotAuthorizedException, BadRequestException {
        log.error("上传文件:{},长度:{},媒体类型:{}",name,totalLength,mediaType);
        //检查是否对这个空间有写入权限
        ...
        for (int i = 0; i <= sliceNum; i++) {
            ...
            byte [] b = in.readNBytes(readLength.intValue());
            if(i == 0){
                //因为不能一次性获取完整数据,所以取第一个分片当做md5的计算,与其他地方的上传有所不同
                fileMd5 = SecureUtil.md5(new String(b));
                //尝试是否可以从垃圾箱恢复
                if(manageFactory.getFileAdministration().dustbinRecovery(fileMd5,userSpace.getId(), FileSourceENUM.CLOUDDISK).isResult()){//从垃圾箱恢复
                    break;
                }
                if(manageFactory.getFileAdministration().checkDuplicate(name,type,mediaType,
                        fileMd5,userSpace.getId(),FileSourceENUM.CLOUDDISK,totalLength,userSpace.getCreateUserId()).isResult()){//引用了其它人上传的文件,秒存
                    break;
                }
            }
            //需要发送分片给文件存储服务
            ...
            if(!StringUtils.hasLength(mediaType) && wh.getData() != null){//如果没有媒体类型,写盘结果中拿,为了提高兼容,写盘的时候如果没有媒体类型,会读取文件真实类型并返回,提高兼容
